...
const Detail = ({ navigation, route }: homeProp) => {
  ...
  useEffect(() => {
    if (inCart && selectedFood) {
      const cartItem = cart.find((c) => c.id === selectedFood.id);
      if (cartItem) {
        setQuantity(cartItem.quantity);
      }
    }
  }, [inCart, cart, selectedFood]);
...
*/}
          ...
          <View style={detailStyle.priceCont}>
            <View style={detailStyle.counterCont}>
              <TouchableOpacity
                ...
                style={detailStyle.counterBtn}
              >
                <AntDesign name="minus" color={"#f10246"} size={48} />
              </TouchableOpacity>
              <Text style={detailStyle.counterNum}>{quantity}</Text>
              <TouchableOpacity
                onPress={() => {
                  if (inCart && selectedFood) {
                    dispatch(
            ...
            <Text style={detailStyle.price}>
              ₵ {parseInt(selectedFood?.price as string).toFixed(2)}
            </Text>
          </View>
          ...
          <View>
            ...
            >
              ...
              >
                <Image
                  source={images.items[5]}
                  style={{ height: 25, width: 25 }}
                />
                <Text style={detailStyle.cartsText}>
                  {inCart ? "Proceed to Carts" : "Add To Carts"}
                </Text>
              </LinearGradient>
            </TouchableOpacity>
          </View>
