...
import { auth, db, firebaseApp } from "../../app/Firebase";
import { Badge, Button, FAB } from "react-native-elements";
import {
  cleanOrders,
  getOrders,
  orderType,
...
const Home = ({ navigation, route }: homeProp) => {
  ...
  const { complete: completeAuthDetails } = useAddAdditionalInfo();
  const {
    accountCreated,
    completeMessage: phoneAuthCompleteMessage,
  ...
  useEffect(() => {
    if (accountCreated || completeAuthDetails) {
  ...
  }, [completeAuthDetails, accountCreated]);
    ...
    return (
      <View style={styles.main}>
        ...
        <AuthModal
          ...
          phoneAuthCompleteMessage={phoneAuthCompleteMessage}
        ...
        <Animated.View
          style={[
            ...
            {
              opacity: fadeOutAnim,
              display: isAnimationDone || user ? "none" : "flex",
            },
          ]}
        >
          <ImageBackground
            source={images.items[6]}
            style={{
              height: "100%",
              width: "100%",
              alignItems: "center",
          ...
          >
            <View
              style={{
                height:
                  screenHeight < 650 ? Sizes.height / 2.2 : Sizes.height / 2.5,
              }}
            >
              <View
                style={[
                  styles.loginInputCont,
                  { marginTop: keyboardShow ? 250 : 60 },
                ]}
              >
        ...
        <View style={styles.header}>
          ...
          >
            <Ionicons name="md-menu-outline" size={28} color={Colors.white} />
          </TouchableOpacity>
            ...
            <Animated.View
              style={[
                styles.imgCont,
                {
                  height: imageSize,
                  width: imageSize,
                  marginTop: slideTopAnim,
                },
              ]}
            >
              <View
                style={[styles.logoImg, { top: isAnimationDone ? 80 : 90 }]}
              >
                <Image
                  source={images.items[2]}
                  style={{
                    height: isAnimationDone ? 47 : 60,
                    width: isAnimationDone ? 158 : 188,
                  }}
                />
              </View>
              <Image
            ...
            {isAnimationDone ? (
              <Animatable.View
                ...
                ref={(el) => {
                  if (removedAnim) {
                    if (el) {
                      (el as any).fadeInUp(1100).then((anim: any) => {
                        anim.finished
                          ? setRemovedAnim(false)
        ...
        <View style={styles.mainContent}>
          {processingOrders.length > 0 && (
            ...
            >
              <Badge value={processingOrders.length} status="success" />
              <FAB
                onPress={() => {
  ...
  return (
    ...
    >
      <ActivityIndicator animating color={Colors.white} size={38} />
    </LinearGradient>
  );
  // return (
